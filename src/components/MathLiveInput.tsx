...
export const MathLiveInput = forwardRef<MathLiveInputHandle, MathLiveInputProps>(({
    ...
    useEffect(() => {
        ...
        const kbdHost = dockRef.current.querySelector('#mathlive-keyboard-container') as HTMLElement | null;
        if (kbdHost && window.mathVirtualKeyboard) {
            // @ts-ignore
            window.mathVirtualKeyboard.container = kbdHost;
            // @ts-ignore
            try { window.mathVirtualKeyboard.connect?.(mfRef.current); } catch { window.mathVirtualKeyboard.connect?.(); }
            if (!readOnly) {
                // @ts-ignore
